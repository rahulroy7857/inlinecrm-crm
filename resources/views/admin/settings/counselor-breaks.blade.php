...
@section('title', 'Counselor Breaks')
@section('style')
...
<div class="container-xxl flex-grow-1 container-p-y crm-page">
                        ...
                        <tr>
                            ...
                            <td class="d-flex flex-wrap gap-2">
                                <button type="button"
                                        ...
                                        data-id="{{ $request->id }}"
                                        data-name="{{ $request->counselor->name }}"
    ...
    <div class="card">
        <div class="card-header border-bottom">
            ...
            <p class="mb-0 small text-muted">Set duration for each break. Types without duration require admin approval before counselor can start the break.</p>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.settings.counselor-breaks.update') }}">
                @csrf
                @method('PUT')
                <div class="table-responsive">
                    <table class="table crm-table">
    ...
    document.querySelectorAll('.approve-break-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            document.getElementById('approveBreakForm').action =
                "{{ route('admin.settings.counselor-breaks.approve', ':id') }}".replace(':id', btn.dataset.id);
    ...
    document.querySelectorAll('.reject-break-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            document.getElementById('rejectBreakForm').action =
                "{{ route('admin.settings.counselor-breaks.reject', ':id') }}".replace(':id', btn.dataset.id);
