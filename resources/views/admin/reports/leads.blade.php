...
@section('content')
@php
    $hasFilters = request()->hasAny(['from_date','to_date','college_id','course_id','source_id','status','counselor_id']);
...
<div class="container-xxl flex-grow-1 container-p-y crm-page crm-report-page">
    <div class="crm-report-hero">
        <div>
            ...
            <p class="crm-report-hero__sub">Filter and review leads by date, college, course, source, and status.</p>
        </div>
        ...
        <div class="card-body">
            <div class="crm-report-filter">
                <div class="crm-report-filter__head">
                    ...
                    @if($hasFilters)
                        <a href="{{ route('admin.reports.leads') }}" class="btn btn-sm btn-outline-secondary">
                            <i class="bx bx-x me-1"></i>Clear all
                        </a>
                    @endif
                </div>
                <div class="crm-report-filter__body">
                    <form action="{{ route('admin.reports.leads') }}" method="GET">
                        <div class="crm-report-filter__grid">
                            ...
                            <div class="crm-report-filter__field">
                                ...
                                <select class="select2-multiple form-select" id="counselor" name="counselor_id[]" multiple data-placeholder="Select counselor">
                                    @foreach($counselors as $counselor)
                                        <option value="{{ $counselor->id }}" {{ in_array($counselor->id, (array) request('counselor_id')) ? 'selected' : '' }}>
                                            {{ $counselor->name }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <table id="leadsTable" class="table crm-table">
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('admin.leads.show', $lead->id) }}" class="btn btn-icon btn-outline-primary">
