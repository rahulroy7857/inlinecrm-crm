...
class CounselorBreakSettingsService
{
    public function allActive(): Collection
    {
        return Cache::remember('counselor_break_settings', 300, function () {
            return CounselorBreakSetting::query()
    ...
    public function findByType(string $type): ?CounselorBreakSetting
    {
        return $this->allActive()->firstWhere('type', $type)
            ?? CounselorBreakSetting::query()->where('type', $type)->first();
    }
    ...
    {
        return $this->allActive()->map(function (CounselorBreakSetting $setting) {
            return [
                'type' => $setting->type,
                'label' => $setting->label,
