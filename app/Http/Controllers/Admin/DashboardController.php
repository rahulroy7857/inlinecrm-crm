...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $leadsCount = [
            'new' => Lead::where('academic_year_id', $this->academicYear)->where('status', 'New')->count(),
            'warm' => Lead::where('academic_year_id', $this->academicYear)->where('status', 'Warm')->count(),
            'hot' => Lead::where('academic_year_id', $this->academicYear)->where('status', 'Hot')->count(),
            'application' => Lead::where('academic_year_id', $this->academicYear)->where('status', 'Application')->count(),
            'admission' => Lead::where('academic_year_id', $this->academicYear)->where('status', 'Admission')->count(),
            'total' => Lead::where('academic_year_id', $this->academicYear)->count(),
        ];
        ...
        $prevLeadsCount = [
            'new' => Lead::where('academic_year_id', $prevAcademicYear)->where('status', 'New')->count(),
            'warm' => Lead::where('academic_year_id', $prevAcademicYear)->where('status', 'Warm')->count(),
            'hot' => Lead::where('academic_year_id', $prevAcademicYear)->where('status', 'Hot')->count(),
            'application' => Lead::where('academic_year_id', $prevAcademicYear)->where('status', 'Application')->count(),
            'admission' => Lead::where('academic_year_id', $prevAcademicYear)->where('status', 'Admission')->count(),
            'total' => Lead::where('academic_year_id', $prevAcademicYear)->count(),
        ];
        ...
        $funnelData = [
            'total' => Lead::where('academic_year_id', $this->academicYear)->count(),
            'positive' => Lead::where('academic_year_id', $this->academicYear)->whereIn('status', ['Warm', 'Hot'])->count(),
            'application' => Lead::where('academic_year_id', $this->academicYear)->where('status', 'Application')->count(),
            'admission' => Lead::where('academic_year_id', $this->academicYear)->where('status', 'Admission')->count()
        ];
        ...
        $topCounselor = Counselor::withCount(['leads' => function($query) {
            $query->where('status', 'Admission');
        }])
        ->orderBy('leads_count', 'desc')
        ->first();

        // Get recent admissions
        $recentAdmissions = Lead::with(['college', 'course'])
        ...
        $recentTransactions = LeadPayment::with('lead')
            ->latest()
            ->limit(6)
            ->get();
        ...
        // Get revenue data
        $revenueData = LeadPayment::select(
        ...
        foreach ($revenueData as $month => $total) {
            $monthlyRevenueData[$month] = $total;
        }
